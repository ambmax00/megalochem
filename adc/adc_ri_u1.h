...
public:
	...
	void compute_r_ovov(dbcsr::stensor<2>& u1, double omega) {
		...
		// compute R_iajb = c_os_coup * [(bar(ia)|jb) + (ia|bar(jb))]
		// /(D_iajb + omega)
...
class lp_ri_adc2_diis_u1 : ri_adc1_u1 {
protected:
...
public:
	...
	lp_ri_adc2_diis_u1(svector<double> eps_o, svector<double> eps_v, 
		stensor<3>& B_xoo, 
		stensor<3>& B_xov, 
		stensor<3>& B_xvv) :
		ri_adc1_u1(eps_o, eps_v, B_xoo, B_xov, B_xvv)
	...
	math::laplace lap(5, *(eps_o->begin()), *(eps_o->end() - 1), 
		*(eps_v->begin()),*(eps_v->end() - 1));
