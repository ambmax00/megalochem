...
dbcsr::stensor<2> eigen_inverse(dbcsr::stensor<2>& t_in, std::string name) {
	...
	auto t_out = dbcsr::eigen_to_tensor(inv, name, grid, vec<int>{0}, vec<int>{1}, t_in->blk_sizes());  
...
dbcsr::stensor<2> eigen_sqrt_inverse(dbcsr::stensor<2>& t_in, std::string name) {
	...
	auto t_out = eigen_to_tensor(inv, name, grid, vec<int>{0}, vec<int>{1}, t_in->blk_sizes());
