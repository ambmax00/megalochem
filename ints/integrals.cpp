...
void calc_ints(math::blockmap<3>& b_in, arrvec<int,3>& newblks, util::ShrPool<libint2::Engine>& engine,
	...
	auto blksizes = b_in.blk_sizes();
	
	for (int iblk = 0; iblk != newblks[0].size(); ++iblk) {
		
		int ix = newblks[0][iblk];
		int im = newblks[1][iblk];
		int in = newblks[2][iblk];
		
		const auto& c1 = cbas1[ix];
		const auto& c2 = cbas2[im];
		const auto& c3 = cbas3[in];
				
		std::cout << "COMPUTING: " << ix << " " << im << " " << in << std::endl;

		std::array<int,3> idx = {ix,im,in};
		double* blk = b_in.get_block(idx);
		
		int locblkoff1 = 0;
		int locblkoff2 = 0;
		int locblkoff3 = 0;
		
		int xsize = blksizes[0][ix];
		int msize = blksizes[1][im];
		int nsize = blksizes[2][in];
