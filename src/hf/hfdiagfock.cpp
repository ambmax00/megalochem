...
void hfmod::diag_fock()
{
  ...
  auto diagonalize = [&](smat_d& f_bb, smat_d& c_bm, std::vector<double>& eps,
                         std::string x) {
    ...
    auto FX = dbcsr::matrix<>::create_template(*f_bb)
                  ...
                  .build();
    auto XFX = dbcsr::matrix<>::create_template(*f_bb).name("XFX").build();
    ...
    math::hermitian_eigen_solver solver(
        m_world, XFX, 'V', (LOG.global_plev() >= 2) ? true : false);
    ...
    auto new_c_bm =
        dbcsr::matrix<>::create_template(*c_bm_x).name(c_bm_x->name()).build();
  ...
  auto form_density = [&](dbcsr::shared_matrix<double>& p_bb,
    ...
    if (x == "A")
      limit = m_mol->nocc_alpha() - 1;
    if (x == "B")
      limit = m_mol->nocc_beta() - 1;
    ...
    LOG.os<1>(
        "Occupancy of ", p_bb->name(), " : ", p_bb->occupation() * 100, "%\n");
  };
  ...
  if (!m_restricted && !m_nobetaorb) {
    ...
    auto p_bb_B =
        dbcsr::matrix<>::create_template(*m_p_bb_A).name("p_bb_B").build();
    m_p_bb_B->set(0.0);
  ...
  if (m_c_bm_B)
    m_c_bm_B->filter(dbcsr::global::filter_eps);
  if (m_p_bb_B)
    m_p_bb_B->filter(dbcsr::global::filter_eps);
}
...
void hfmod::compute_virtual_density()
{
  auto form_density = [&](dbcsr::shared_matrix<double>& pv_bb,
    ...
    pv_bb =
        dbcsr::matrix<>::create_template(*m_p_bb_A).name("pv_bb_" + x).build();
  ...
  if (m_mol->nvir_alpha() != 0) {
    ...
    m_pv_bb_A =
        dbcsr::matrix<>::create_template(*m_p_bb_A).name("pv_bb_A").build();
  ...
  if (!m_restricted && m_mol->nele_beta() != 0) {
    ...
    m_pv_bb_B =
        dbcsr::matrix<>::create_template(*m_p_bb_A).name("pv_bb_B").build();
