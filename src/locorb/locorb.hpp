...
class mo_localizer {
 ...
 public:
  mo_localizer(world w, desc::shared_molecule mol) :
      m_world(w), m_mol(mol),
      m_aofac(std::make_shared<ints::aofactory>(mol, w)), LOG(w.comm(), 0)
  {
  }
  ...
  dbcsr::shared_matrix<double> compute_conversion(
      dbcsr::shared_matrix<double> c_bm,
      dbcsr::shared_matrix<double> s_bb,
      dbcsr::shared_matrix<double> l_bm)
  {
    auto temp = dbcsr::matrix<>::create_template(*c_bm).name("temp").build();
  ...
  // double pop_mulliken(dbcsr::matrix<double>& c_bo, dbcsr::matrix<double>&
  // s_bb, 	dbcsr::matrix<double>& s_sqrt_bb);
};
