...
std::tuple<smat_d, smat_d> mo_localizer::compute_pao(smat_d c_bm, smat_d s_bb)
{
  ...
  auto u_bm = dbcsr::matrix<>::create_template(*c_bm).name("u_bm").build();
    ...
    smat_d u_km)
{
  ...
  for (auto iblk : blkidx) { p.push_back(b[iblk]); }
  ...
  auto cortho_bm =
      dbcsr::matrix<>::create_template(*c_bm).name("c_ortho").build();
  ...
  dbcsr::multiply('N', 'N', 1.0, *s_inv_pp, *s_pb, 0.0, *temp_pb).perform();

  dbcsr::multiply('N', 'N', 1.0, *s_sqrt_bb, *c_bm, 0.0, *cortho_bm).perform();

  dbcsr::multiply('N', 'N', 1.0, *temp_pb, *cortho_bm, 0.0, *q_pm).perform();
  ...
  dbcsr::multiply('N', 'N', 1.0, *vt_rm, *f_mm, 0.0, *f_ht_rm).perform();

  dbcsr::multiply('N', 'T', 1.0, *f_ht_rm, *vt_rm, 0.0, *f_rr).perform();

  math::hermitian_eigen_solver hermsolver(
      m_world, f_rr, 'V', LOG.global_plev());
  hermsolver.compute();
  ...
  if (m_world.rank() == 0) {
    ...
    for (auto e : eps_m) { std::cout << e << " "; }
    std::cout << std::endl;
    std::cout << "New molecular energies: " << std::endl;
    for (auto e : eps_s) { std::cout << e << " "; }
    std::cout << std::endl;
  }
  ...
  dbcsr::multiply('T', 'N', 1.0, *vt_rm, *T_rs, 0.0, *T_ms).perform();
        ...
        dbcsr::matrix<double>& s_bb, dbcsr::matrix<double>& s_sqrt_bb)
{
        ...
        MPI_Allreduce(MPI_IN_PLACE, &nele_loc, 1, MPI_DOUBLE, MPI_SUM,
w.comm());
