...
moprintmod::moprintmod(moprintmod::create_pack&& p) :
    m_world(p.p_set_world), m_wfn(p.p_set_wfn), m_filename(p.p_filename),
    m_job_name(p.p_job_name), m_lmo_occ(p.p_lmo_occ), m_lmo_vir(p.p_lmo_vir),
...
void moprintmod::compute()
{
  ...
  if (m_job_name == job_type::cmo) {
    ...
    io::write_molden(
        "cmo_" + m_filename, m_world, *mol, cOA, cVA, *eOA, *eVA, cOB, cVB,
        eOB ? *eOB : std::vector<double>{}, eVB ? *eVB : std::vector<double>{});
    return;
  }
  ...
  if (m_job_name == job_type::local) {
    ...
    auto localize = [&](auto ltype, auto c_bm, auto eps_m) {
      ...
      dbcsr::multiply('N', 'T', 1.0, *f_mm, *u_rm, 0.0, *f_mr).perform();

      dbcsr::multiply('N', 'N', 1.0, *u_rm, *f_mr, 0.0, *f_rr).perform();
    ...
    io::write_molden(
        "lmo_" + m_filename, m_world, *mol, lcOA, lcVA, leOA, leVA, cOB, cVB,
        eOB ? *eOB : std::vector<double>{}, eVB ? *eVB : std::vector<double>{});
    return;
  }
  ...
  if (m_job_name == job_type::nto) {
    ...
    for (int istate = 0; istate != nstates; ++istate) {
      auto r_ov = dbcsr::matrix<double>::copy(*eigvecs[istate]).build();
      ...
      std::for_each(epso_r.begin(), epso_r.end(), [](double& d) {
        d = -d;
      });
      ...
      io::write_molden(
          filename, m_world, *mol, co_br, cv_br, epso_r, epsv_r, nullptr,
          nullptr, std::vector<double>{}, std::vector<double>{});
      return;
    }
  }
}
