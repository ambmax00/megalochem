...
private:
	...
	void next_perm_even() {
		...
		MPI_Sendrecv(&m_idx_u.back(), 1, MPI_INT, right, 0,	
			&m_idx_u.front(), 1, MPI_INT, left, 0, m_comm,
			MPI_STATUS_IGNORE);
			
		if (m_rank == 0) m_idx_u.front() = -1;
		
		// send cols
		double* last_col = m_mat_u.data() + m_totnrows * (m_npairs);
		double* first_col = m_mat_u.data(); 
		
		MPI_Sendrecv(last_col, m_totnrows, MPI_DOUBLE, right, 2,
			first_col, m_totnrows, MPI_DOUBLE, left, 2, m_comm,
			MPI_STATUS_IGNORE);
			
		if (m_rank != m_last) m_idx_u.back() = -1;
	...
	void next_perm_odd() {
		...
		// shuffle indices
		for (int i = 0; i != m_npairs; ++i) {
			m_idx_l[i] = m_idx_l[i+1];
		}
		
		// shuffle cols
		for (int i = 0; i != m_npairs; ++i) {
			m_mat_l.col(i) = m_mat_l.col(i+1);
		}
		...
		MPI_Sendrecv(&m_idx_l.front(), 1, MPI_INT, left, 1,
			&m_idx_l.back(), 1, MPI_INT, right, 1, m_comm,
			MPI_STATUS_IGNORE);
			
		if (m_rank == 0) m_idx_u.front() = m_idx_l.front();
		if (m_rank == m_last) m_idx_l.back() = m_idx_u.back();
		
		// send cols
		double* first_col = m_mat_l.data();
		double* last_col = m_mat_l.data() + m_totnrows * (m_npairs);
		
		MPI_Sendrecv(first_col, m_totnrows, MPI_DOUBLE, left, 4,
			last_col, m_totnrows, MPI_DOUBLE, right, 4, m_comm,
			MPI_STATUS_IGNORE);
			
		if (m_rank == 0) m_mat_u.col(0) = m_mat_l.col(0);
		if (m_rank == m_last) m_mat_l.col(m_npairs) = m_mat_u.col(m_npairs);
...
public:
		...
		: m_comm(comm), LOG(comm,0) 
	{
		...
		for (int i = 0; i != m_mpisize; ++i) {
			m_npairs_proc[i] = cat_size / m_mpisize 
				+ ((i == 0) ? cat_size % m_mpisize : 0);
		...
		m_npairs_tot = std::accumulate(m_npairs_proc.begin(),
			m_npairs_proc.end(), 0);
	...
	void next_perm() {
		...
		if (m_iperm % 2 == 0) next_perm_even();
		if (m_iperm % 2 == 1) next_perm_odd();
				
		++m_iperm;
		
	}
	
	int npairs() { return m_npairs; }
	
	int npairs_tot() { return m_npairs_tot; }
	
	std::vector<int> npairs_proc() { return m_npairs_proc; }
	
	std::vector<int> npairs_off() { return m_npairs_off; }
	...
	Eigen::Block<Eigen::MatrixXd> upper_col(int idim, int ipair) {
		//std::cout << "NDIM: " << idim << " offset: " << m_row_offset[idim] << std::endl;
	...
	std::array<Eigen::MatrixXd,DIM> get_mats() {
		...
		for (int imat = 0; imat != DIM; ++imat) {
			...
			for (int iproc = 0; iproc != m_mpisize; ++iproc) {
				...
				for (int ipair = 0; ipair != m_npairs_proc[iproc]+1; 
					++ipair) 
				{
					...
					if (iproc == m_rank) {
						...
						idx[1] = (ipair == m_npairs_proc[iproc]) ?
							 -1 : this->lower_pair(ipair);
					}
					...
					if (idx[0] != -1 && iproc == m_rank) {
						mat.col(idx[0]) = 
							m_mat_u.block(m_row_offset[imat],ipair,
								m_nrows[imat],1);
					}
					
					if (idx[1] != -1 && iproc == m_rank) {
						mat.col(idx[1]) = 
							m_mat_l.block(m_row_offset[imat],ipair+1,
								m_nrows[imat],1);
					}
...
void update_mat(Eigen::Block<Eigen::MatrixXd> col_i, 
	Eigen::Block<Eigen::MatrixXd> col_j, int i, int j, int ncol,
	double omega) 
{		
	double ca = cos(omega);
	double sa = sin(omega);
...
void form_type() {
	...
	MPI_Type_create_struct(num, ele_blklength, array_of_offsets, 
		array_of_types, &MPI_ROT_INFO);
		
	MPI_Aint lb, extent;
	MPI_Type_get_extent(MPI_ROT_INFO, &lb, &extent);
	if (extent != sizeof(myrot)) {
		MPI_Datatype old = MPI_ROT_INFO;
		MPI_Type_create_resized(old, 0, sizeof(myrot), 
			&MPI_ROT_INFO);
		MPI_Type_free(&old);
	}
	...
	*/
	...
	for (int iperm = 0; iperm != norb + norb%2; ++iperm) {
		...
		for (int ipair = 0; ipair != npairs; ++ipair) {
			...
			double A = pow(xcol_i(j,0),2.0) + pow(ycol_i(j,0),2.0) + pow(zcol_i(j,0),2.0)
					- 0.25 * (pow(xcol_i(i,0),2.0) + pow(ycol_i(i,0),2.0) + pow(zcol_i(i,0),2.0))
					- 0.25 * (pow(xcol_j(j,0),2.0) + pow(ycol_j(j,0),2.0) + pow(zcol_j(j,0),2.0))
					+ 0.5 * (xcol_i(i,0) * xcol_j(j,0) + ycol_i(i,0) * ycol_j(j,0) 
		...
		MPI_Allgatherv(rinfo_vec_local.data(), npairs, MPI_ROT_INFO,
			rinfo_vec_global.data(), npairs_proc.data(), 
			npairs_off.data(), MPI_ROT_INFO, w.comm());
		...
		for (int ipair = 0; ipair != npairs_tot; ++ipair) {
			...
			if (i == -1 || j == -1 || fabs(omega) < t12) continue;
	...
	x_dist = dbcsr::eigen_to_matrix(new_mats[0], w, "x_mult", m, m, dbcsr::type::symmetric);
	y_dist = dbcsr::eigen_to_matrix(new_mats[1], w, "y_mult", m, m, dbcsr::type::symmetric);
	z_dist = dbcsr::eigen_to_matrix(new_mats[2], w, "z_mult", m, m, dbcsr::type::symmetric);
	c_dist = dbcsr::eigen_to_matrix(new_mats[3], w, "c_bm", b, m, dbcsr::type::no_symmetry);
