...
#define BTENSOR_TEMPLATE_PLIST (\
	...
	bool fits_in_mem(std::vector<int> dims) {
		...
		for (int ii = 0; ii != N; ++ii) {
			...
			ntot *= std::accumulate(m_blk_sizes[ii].begin(),
				m_blk_sizes[ii].end(),(int64_t)0);
		...
		int64_t ntot_per_proc_batch = (ntot / (int64_t)m_mpisize) / 
			(int64_t) nbatches;
	...
	void print_info() {
		...
		LOG.os<>("Sparsity info of batch tensor ", m_name, '\n',
