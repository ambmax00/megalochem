...
#define DIST_LIST \
  (((cart), set_cart), ((vec<int>), row_dist), ((vec<int>), col_dist))

  MAKE_PARAM_STRUCT(create, DIST_LIST, ())

  MAKE_BUILDER_CLASS(dist, create, DIST_LIST, ())

  dist(create_pack&& p) : m_dist_ptr(nullptr), m_cart(p.p_set_cart)
  {
    c_dbcsr_distribution_new(
        &m_dist_ptr, p.p_set_cart.group(), p.p_row_dist.data(),
        p.p_row_dist.size(), p.p_col_dist.data(), p.p_col_dist.size());
  }
...
#define MATRIX_CREATE_LIST \
  (((std::string), name), ((util::optional<cart>), set_cart), \
   ((util::optional<dist&>), set_dist), ((type), matrix_type), \
   ((vec<int>&), row_blk_sizes), ((vec<int>&), col_blk_sizes), \
   ((util::optional<int>), nze), ((util::optional<bool>), reuse), \
  ...
  matrix(create_pack&& p) :
      ...
      m_cart((p.p_set_cart) ? (*p.p_set_cart) : (*p.p_set_dist).m_cart)
  {
    ...
    if (!p.p_set_dist) {
      ...
      auto rdist =
          default_dist(p.p_row_blk_sizes.size(), dims[0], p.p_row_blk_sizes);
      auto cdist =
          default_dist(p.p_col_blk_sizes.size(), dims[1], p.p_col_blk_sizes);

      dist_ptr = new dist(std::move(*dist::create()
    ...
    char repl_type =
        (p.p_replication_type) ? static_cast<char>(*p.p_replication_type) : '0';

    c_dbcsr_create_new(
        &m_matrix_ptr, p.p_name.c_str(), dist_ptr->m_dist_ptr, mat_type,
...
#define MATRIX_TEMPLATE_ILIST (((matrix<T>&), templet))

#define MATRIX_TEMPLATE_PLIST \
  (((std::string), name), ((util::optional<dist&>), set_dist), \
   ((util::optional<type>), matrix_type), \
   ((util::optional<vec<int>&>), row_blk_sizes), \
   ((util::optional<vec<int>&>), col_blk_sizes), ((util::optional<int>), nze), \
   ((util::optional<bool>), reuse), ((util::optional<bool>), reuse_arrays), \
   ((util::optional<bool>), mutable_work), \
   ((util::optional<repl>), replication_type))

  MAKE_PARAM_STRUCT(
      create_template, MATRIX_TEMPLATE_PLIST, MATRIX_TEMPLATE_ILIST)

  MAKE_BUILDER_CLASS(
      matrix, create_template, MATRIX_TEMPLATE_PLIST, MATRIX_TEMPLATE_ILIST)
  ...
  matrix(create_template_pack&& p) : m_cart(p.p_templet.m_cart)
  {
    ...
    char mat_type =
        (p.p_matrix_type) ? static_cast<char>(*p.p_matrix_type) : '0';
    char repl_type =
        (p.p_replication_type) ? static_cast<char>(*p.p_replication_type) : '0';

    c_dbcsr_create_template(
        &m_matrix_ptr, p.p_name.c_str(), p.p_templet.m_matrix_ptr,
...
#define MATRIX_TRANSPOSE_ILIST (((matrix<T>&), matrix_in))
...
#define MATRIX_TRANSPOSE_PLIST \
  ...
  MAKE_PARAM_STRUCT(transpose, MATRIX_TRANSPOSE_PLIST, MATRIX_TRANSPOSE_ILIST)

  MAKE_BUILDER_CLASS(
      matrix, transpose, MATRIX_TRANSPOSE_PLIST, MATRIX_TRANSPOSE_ILIST)
  ...
  matrix(transpose_pack&& p) : m_cart(p.p_matrix_in.m_cart)
  {
    ...
    c_dbcsr_transposed(
        &m_matrix_ptr, p.p_matrix_in.m_matrix_ptr,
...
#define MATRIX_COPY_ILIST (((matrix<T>&), matrix_in))
...
#define MATRIX_COPY_PLIST \
  ...
  MAKE_BUILDER_CLASS(matrix, copy, MATRIX_COPY_PLIST, MATRIX_COPY_ILIST)
...
#define MATRIX_READ_PLIST \
  (((std::string), filepath), ((dist&), set_dist), ((cart), set_cart))
  ...
  matrix(read_pack&& p) : m_cart(p.p_set_cart)
  {
    ...
    c_dbcsr_binary_read(
        p.p_filepath.c_str(), p.p_distribution.m_dist_ptr, m_cart.comm(),
        &m_matrix_ptr);
  }
  ...
  {
    c_dbcsr_scale_by_vector(
        m_matrix_ptr, alpha.data(), alpha.size(), side.c_str());
  }
      ...
      std::optional<T> assume_value = std::nullopt)
  {
    c_dbcsr_hadamard_product(
        a.m_matrix_ptr, b.m_matrix_ptr, m_matrix_ptr,
        (assume_value) ? &*assume_value : nullptr);
  }
  ...
  block<2, T> get_block_p(const int row, const int col, bool& found) const
  {
    ...
    c_dbcsr_get_block_p(
        m_matrix_ptr, row, col, &data, &found, &size[0], &size[1]);
  ...
  void complete_redistribute(
      matrix<T>& in,
      std::optional<bool> keep_sparsity = std::nullopt,
      std::optional<bool> summation = std::nullopt)
  {
    c_dbcsr_complete_redistribute(
        in.m_matrix_ptr, m_matrix_ptr,
      ...
      std::optional<bool> filter_diag = std::nullopt)
  {
    ...
    c_dbcsr_filter(
        m_matrix_ptr, &eps, (method) ? &method_int : nullptr,
  ...
  void copy_in(
      const matrix& in,
      std::optional<bool> keep_sp = std::nullopt,
      ...
      std::optional<char> type = std::nullopt)
  {
    ...
    c_dbcsr_copy(
        &m_matrix_ptr, in.m_matrix_ptr, name.c_str(),
        (keep_sp) ? &*keep_sp : nullptr, (shallow) ? &*shallow : nullptr,
        (keep_im) ? &*keep_im : nullptr, (type) ? &*type : nullptr);
  }
  ...
  void reserve_sym()
  {
    ...
    if (nrowblk != ncolblk)
      throw std::runtime_error(
          "Cannot allocate matrix blocks in a symmatric fashion.");
  ...
  {
    c_dbcsr_put_block2d(
        m_matrix_ptr, row, col, blk.data(), blk.size()[0], blk.size()[1],
  ...
  {
    c_dbcsr_put_block2d(
        m_matrix_ptr, row, col, data, rowsize, colsize, (sum) ? &*sum : nullptr,
        (scale) ? &*scale : nullptr);
  }

  T* data(
      long long int& data_size,
      std::optional<int> lb = std::nullopt,
      std::optional<int> ub = std::nullopt) const
  {
    T data_type = T();
    T* ptr = nullptr;
    c_dbcsr_get_data(
        m_matrix_ptr, &ptr, &data_size, &data_type, (lb) ? &*lb : nullptr,
        (ub) ? &*ub : nullptr);
    return ptr;
  }
  ...
  {
    c_dbcsr_init_random(
        m_matrix_ptr, (keep_sparsity) ? &*keep_sparsity : nullptr);
  }
  ...
  {
    c_dbcsr_complete_redistribute(
        mat_in.m_matrix_ptr, m_matrix_ptr,
        (keep_sparsity) ? &*keep_sparsity : nullptr,
        (summation) ? &*summation : nullptr);
  }
  ...
  std::string name() const
  {
    char* cname;
    c_dbcsr_get_info(
        m_matrix_ptr, REPEAT_FIRST(ECHO_P, nullptr, 1, 19, (PPDIRS_COMMA), ()),
  ...
  type matrix_type() const
  {
    char out;
    c_dbcsr_get_info(
        m_matrix_ptr, REPEAT_FIRST(ECHO_P, nullptr, 1, 20, (PPDIRS_COMMA), ()),
        &out, nullptr, nullptr);
      ...
      std::optional<double> a2 = std::nullopt)
  {
    ...
    c_dbcsr_function_of_elements(
        m_matrix_ptr, f, (a0) ? &*a0 : nullptr, (a1) ? &*a1 : nullptr,
        (a2) ? &*a2 : nullptr);
  }
 ...
 public:
  ...
  void start()
  {
    c_dbcsr_iterator_start(
        &m_iter_ptr, m_matrix_ptr, nullptr, nullptr, nullptr, nullptr, nullptr);
  ...
  void next_block_index()
  {
    c_dbcsr_iterator_next_block_index(
        m_iter_ptr, &m_row, &m_col, &m_iblk, &m_blk_p);
  }

  void next_block()
  {
    c_dbcsr_iterator_next_2d_block(
        m_iter_ptr, &m_row, &m_col, &m_blk_ptr, &m_transposed, &m_iblk,
