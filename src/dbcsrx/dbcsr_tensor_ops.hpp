...
D* unfold_bounds(vec<vec<D>>& v)
{
  ...
  for (int j = 0; j != b_size; ++j) {
    for (int i = 0; i != 2; ++i) { f_bounds[i + j * 2] = v[j][i]; }
  }
  return f_bounds;
}
...
#define TENSOR_COPY_LIST \
  (((util::optional<vec<int>>), order), ((util::optional<bool>), sum), \
 ...
 public:
  ...
  void perform()
  {
    ...
    c_dbcsr_t_copy(
        c_t_in.m_tensor_ptr, N, c_t_out.m_tensor_ptr,
        (c_order) ? c_order->data() : nullptr, (c_sum) ? &*c_sum : nullptr,
        fbounds, (c_move_data) ? &*c_move_data : nullptr, nullptr);
...
inline tensor_copy_base<tensortype::dim, typename tensortype::value_type> copy(
    tensortype& t1, tensortype& t2)
{
...
#define TENSOR_CONTRACT_LIST \
  (((vec<int>), con1), ((vec<int>), con2), ((vec<int>), ncon1), \
   ((vec<int>), ncon2), ((vec<int>), map1), ((vec<int>), map2), \
   ...
   ((util::optional<long long int&>), flop), ((util::optional<bool>), move), \
   ((util::optional<bool>), retain_sparsity), ((util::optional<bool>), print), \
 ...
 public:
  ...
  contract_base(
      T alpha,
      dbcsr::tensor<N1, T>& t1,
      dbcsr::tensor<N2, T>& t2,
      T beta,
      dbcsr::tensor<N3, T>& t3) :
      c_alpha(alpha),
      c_t1(t1), c_t2(t2), c_beta(beta), c_t3(t3)
  {
  }
  ...
  void perform()
  {
    ...
    if ((c_bounds1 && c_bounds1->size() != c_con1->size()) ||
    ...
    c_dbcsr_t_contract_r_dp(
        c_alpha, c_t1.m_tensor_ptr, c_t2.m_tensor_ptr, c_beta,
        c_t3.m_tensor_ptr, c_con1->data(), c_con1->size(), c_ncon1->data(),
        c_ncon1->size(), c_con2->data(), c_con2->size(), c_ncon2->data(),
        c_ncon2->size(), c_map1->data(), c_map1->size(), c_map2->data(),
        c_map2->size(), f_b1, f_b2, f_b3, nullptr, nullptr, nullptr, nullptr,
        (c_filter) ? &*c_filter : nullptr, (c_flop) ? &(*c_flop) : nullptr,
        (c_move) ? &*c_move : nullptr,
        (c_retain_sparsity) ? &*c_retain_sparsity : nullptr, unit_nr,
        (c_log) ? &*c_log : nullptr);
  ...
  arrvec<int, N3> get_index()
  {
    ...
    c_dbcsr_t_contract_index_r_dp(
        (c_alpha) ? *c_alpha : 1, c_t1.m_tensor_ptr, c_t2.m_tensor_ptr,
        (c_beta) ? *c_beta : 0, c_t3.m_tensor_ptr, c_con1->data(),
        c_con1->size(), c_ncon1->data(), c_ncon1->size(), c_con2->data(),
        c_con2->size(), c_ncon2->data(), c_ncon2->size(), c_map1->data(),
        c_map1->size(), c_map2->data(), c_map2->size(), f_b1, f_b2, f_b3,
        (c_filter) ? &*c_filter : nullptr, &nblkloc, indices, nblkmax, N3);
    ...
    for (int i = 0; i != N3; ++i) {
      ...
      std::copy(
          indices + i * nblkmax, indices + i * nblkmax + nblkloc,
          idx_out[i].data());
    }
  ...
  void eval(std::string str)
  {
    ...
    for (size_t ic = 0; ic != str.size(); ++ic) {
      ...
      if (c == ',') {
        if (++i > 2)
          throw std::runtime_error("Invalid synatax: " + str);
      }
      else if ((c == '-')) {
        if (str[++ic] == '>') {
          ++i;
    ...
    if (idxs[2].size() == 0)
      throw std::runtime_error("Implicit mode not implemented.");
    ...
    */
    ...
    if (ncon1.size() + ncon2.size() != t3.size())
      throw std::runtime_error("Wrong tensor dimensions: " + str);
...
template <int N1, int N2, int N3, typename T>
contract_base(
    T alpha, tensor<N1, T>& t1, tensor<N2, T>& t2, T beta, tensor<N3, T>& t3)
    -> contract_base<
        tensor<N1, T>::dim,
        tensor<N2, T>::dim,
        tensor<N3, T>::dim,
...
template <typename tensor1, typename tensor2, typename tensor3, typename T>
inline contract_base<
    tensor1::dim,
    tensor2::dim,
    tensor3::dim,
    typename tensor1::value_type>
contract(T alpha, tensor1& t1, tensor2& t2, T beta, tensor3& t3)
{
  return contract_base(alpha, t1, t2, beta, t3);
}

template <typename T = double>
void copy_2Dtensor_to_3Dtensor(
    tensor<2, T>& t2, tensor<3, T>& t3, bool sum = false)
{
  iterator_t<2> it2(t2);
  index<3> idx3d;
  index<3> size3;
  ...
  for (int i = 0; i != 3; ++i) { resblkidx[i] = vec<int>(1); }
...
template <typename T = double>
void copy_3Dtensor_to_2Dtensor(
    tensor<3, T>& t3, tensor<2, T>& t2, bool sum = false)
{
  iterator_t<3> it3(t3);
  index<2> idx2d;
  index<2> size2;
  ...
  for (int i = 0; i != 2; ++i) { resblkidx[i] = vec<int>(1); }
...
template <typename T>
void copy_matrix_to_3Dtensor_new(
    matrix<T>& m, tensor<3, T>& t, bool sym = false)
{
  auto w = m.get_cart();
  ...
  for (int ip = 0; ip != mpisize; ++ip) {
    MPI_Gather(
        &send_nblk_p[ip], 1, MPI_INT, recv_nblk_p.data(), 1, MPI_INT, ip, comm);

    MPI_Gather(
        &send_nze_p[ip], 1, MPI_INT, recv_nze_p.data(), 1, MPI_INT, ip, comm);
  }
  ...
  for (auto& v : recv_blkidx) { v.resize(recv_blktot); }

  // send over block indices

  for (int ip = 0; ip != mpisize; ++ip) {
    MPI_Gatherv(
        send_idx0_p[ip].data(), send_nblk_p[ip], MPI_INT, recv_blkidx[0].data(),
        recv_nblk_p.data(), recv_blk_offset.data(), MPI_INT, ip, comm);

    MPI_Gatherv(
        send_idx1_p[ip].data(), send_nblk_p[ip], MPI_INT, recv_blkidx[1].data(),
        recv_nblk_p.data(), recv_blk_offset.data(), MPI_INT, ip, comm);
  }
  ...
  for (int ip = 0; ip != mpisize; ++ip) {
    MPI_Gatherv(
        send_blk_data[ip].data(), send_nze_p[ip], MPI_DOUBLE,
...
void copy_3Dtensor_to_matrix_new(tensor<3, T>& t, matrix<T>& m)
{
  ...
  if (sym) {
    ...
    std::copy(recv_blkidx[0].begin(), recv_blkidx[0].end(), rowres.begin());
    std::copy(recv_blkidx[1].begin(), recv_blkidx[1].end(), colres.begin());
  }
...
void ewmult(tensor<N, T>& t1, tensor<N, T>& t2, tensor<N, T>& tout)
{
  ...
  // make sure tensors have same grid and dimensions, caus I sure don't do it
  // here
  ...
  while (it.blocks_left()) {
    ...
    if (!found3) {
      arrvec<int, N> res;
      for (int i = 0; i != N; ++i) { res[i].push_back(idx[i]); }

      tout.reserve(res);
    }

    std::transform(
        b1.data(), b1.data() + b1.ntot(), b2.data(), b3.data(),
        std::multiplies<T>());
...
void copy_local_to_global(tensor<N, T>& t_loc, tensor<N, T>& t_glob)
{
  ...
  while (iter.blocks_left()) {
    ...
    int nze =
        std::accumulate(&size[0], &size[0] + N, 1, std::multiplies<int>());
    ...
    for (int i = 0; i != N; ++i) { send_idx_p[dest_p][i].push_back(idx[i]); }
  }
  ...
  for (int ip = 0; ip != mpisize; ++ip) {
    MPI_Gather(
        &send_nblk_p[ip], 1, MPI_INT, recv_nblk_p.data(), 1, MPI_INT, ip,
        comm_glob);

    MPI_Gather(
        &send_nze_p[ip], 1, MPI_INT, recv_nze_p.data(), 1, MPI_INT, ip,
        comm_glob);
  }
  ...
  for (int ip = 0; ip != mpisize; ++ip) {
    for (int in = 0; in != N; ++in) {
      MPI_Gatherv(
          send_idx_p[ip][in].data(), send_nblk_p[ip], MPI_INT,
