...
std::tuple<Eigen::VectorXq, Eigen::VectorXq> minimax::para_opt(
    Eigen::VectorXq& expts, Eigen::VectorXq omegas, Eigen::VectorXq alphas)
{
  ...
  while (niter < _max_iter_newton) {
    ...
    for (int ii = 0; ii != 2 * k; ++ii) {
      f(ii) =
          eta(expts(ii), omegas, alphas) + eta(expts(ii + 1), omegas, alphas);
    }
  ...
  if (niter == _max_iter_newton) {
    throw std::runtime_error(
        "MINIMAX: Multi-dimensional Newton did not converge");
  }
