...
std::tuple<Eigen::VectorXq, Eigen::VectorXq> minimax::read_guess(
    double R, int k)
{
  ...
  for (int ii = 0; ii != 4; ++ii) { std::getline(infile, line); }
  ...
  if (R0_chosen < 0.0 && k_recommended != 0) {
    throw std::runtime_error(
        "R value too small. "
        "Use " +
        std::to_string(k_recommended) + " quadrature points instead.");
  }
