...
void SVD::compute(double eps)
{
  ...
  auto sca_mat_in =
      std::make_shared<scalapack::distmat<double>>(dbcsr::matrix_to_scalapack(
          m_mat_in, sgrid, nb, nb, ori_coord[0], ori_coord[1]));
  ...
  c_pdgesvd(
      m_jobu, m_jobvt, m, n, a, 0, 0, desca.data(), s, u, 0, 0, descu.data(),
      vt, 0, 0, descvt.data(), &wsize, -1, &info);
  ...
  c_pdgesvd(
      m_jobu, m_jobvt, m, n, a, 0, 0, desca.data(), s, u, 0, 0, descu.data(),
      vt, 0, 0, descvt.data(), work, lwork, &info);
  ...
  if (m_jobu == 'V') {
    ...
    c_pdgeadd(
        'N', m, m_rank, 1.0, m_U->data(), 0, 0, m_U->desc().data(), 0.0,
        newU->data(), 0, 0, newU->desc().data());
  ...
  if (m_jobvt == 'V') {
    ...
    c_pdgeadd(
        'N', m_rank, n, 1.0, m_Vt->data(), 0, 0, m_Vt->desc().data(), 0.0,
        newVt->data(), 0, 0, newVt->desc().data());
...
dbcsr::shared_matrix<double> SVD::inverse()
{
  ...
  dbcsr::shared_matrix<double> U =
      dbcsr::scalapack_to_matrix(*m_U, dcart, "U", rowsizes, sizes);

  dbcsr::shared_matrix<double> Vt =
      dbcsr::scalapack_to_matrix(*m_Vt, dcart, "Vt", sizes, colsizes);
  ...
  std::for_each(sigma.begin(), sigma.end(), [](double& e) {
...
dbcsr::shared_matrix<double> SVD::U(
    std::vector<int> rowblksizes, std::vector<int> colblksizes)
{
  ...
  auto out = dbcsr::scalapack_to_matrix(
      *m_U, dcart, "SVD U matrix of " + m_mat_in->name(), rowblksizes,
      colblksizes);
...
dbcsr::shared_matrix<double> SVD::Vt(
    std::vector<int> rowblksizes, std::vector<int> colblksizes)
{
  auto dcart = m_mat_in->get_cart();

  auto out = dbcsr::scalapack_to_matrix(
      *m_Vt, dcart, "SVD Vt matrix of " + m_mat_in->name(), rowblksizes,
      colblksizes);
