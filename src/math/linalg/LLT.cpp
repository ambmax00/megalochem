...
void LLT::compute()
{
  ...
  m_L = std::make_shared<scalapack::distmat<double>>(dbcsr::matrix_to_scalapack(
...
dbcsr::shared_matrix<double> LLT::L(vec<int> blksizes)
{
  auto out = dbcsr::scalapack_to_matrix(
      *m_L, m_world.dbcsr_grid(),
      "Cholesky decomposition of " + m_mat_in->name(), blksizes, blksizes,
      "lowtriang");
...
void LLT::compute_L_inverse()
{
  if (!m_L_inv) {
    ...
    c_pdgeadd(
        'N', n, n, 1.0, m_L->data(), 0, 0, m_L->desc().data(), 0.0,
    ...
    c_pdtrtri(
        'L', 'N', n, m_L_inv->data(), 0, 0, m_L_inv->desc().data(), &info);
...
dbcsr::shared_matrix<double> LLT::L_inv(vec<int> blksizes)
{
  ...
  auto out = dbcsr::scalapack_to_matrix(
      *m_L_inv, w, "Inverted cholesky decomposition of " + m_mat_in->name(),
...
dbcsr::shared_matrix<double> LLT::inverse(vec<int> blksizes)
{
  ...
  dbcsr::multiply('T', 'N', 1.0, *Linv, *Linv, 0.0, *out).perform();
