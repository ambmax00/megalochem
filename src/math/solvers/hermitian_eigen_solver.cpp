...
void hermitian_eigen_solver::compute()
{
  ...
  if (m_jobz == 'N') {
    ...
    lwork = 5 * n + std::max(2 * n, (3 + A) * nb * nb) +
        nb * ((n - 1) / (nb * nprow * npcol) + 1) * n + 1;
  }
  ...
  scalapack::distmat<double> sca_mat_in = dbcsr::matrix_to_scalapack(
  ...
  c_pdsyev(
      m_jobz, 'U', n, a_ptr, 0, 0, sca_desc.data(), m_eigval.data(), z_ptr, 0,
      0, sca_desc.data(), work, lwork, &info);
  ...
  if (sca_eigvec_opt) {
    ...
    std::vector<int> rowblksizes =
        (m_rowblksizes_out) ? *m_rowblksizes_out : m_mat_in->row_blk_sizes();

    std::vector<int> colblksizes =
        (m_colblksizes_out) ? *m_colblksizes_out : m_mat_in->col_blk_sizes();

    // sca_eigvec_opt->print();

    m_eigvec = dbcsr::scalapack_to_matrix(
        *sca_eigvec_opt, m_cart, "eigenvectors", rowblksizes, colblksizes);
...
smatrix hermitian_eigen_solver::inverse()
{
  ...
  dbcsr::multiply('N', 'T', 1.0, *eigvec_copy, *m_eigvec, 0.0, *inv).perform();
