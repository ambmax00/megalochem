 ...
 public:
  ...
  void compute(std::vector<smat>& guess, int nroot, double omega)
  {
    ...
    for (int ii = 0; ii != m_macro_maxiter; ++ii) {
      ...
      if (resnorms[nroot - 1] < m_macro_conv && err < m_macro_conv)
        break;
    }
    ...
    /*for (int ii = 0; ii != m_diis_maxiter; ++ii) {
            ...
            if (ii == 0) {
                    ...
                    dsolver.compute_extrapolation_parameters(prev_u, uerr_ov,
    ii);
    ...
    for (int iiter = 0; iiter != m_diis_maxiter; ++iiter) {
      ...
      for (int ii = 0; ii <= iiter; ++ii) {
        for (int jj = ii; jj <= iiter; ++jj) {
          bmat(ii, jj) = bmat(jj, ii) = errvecs[ii]->dot(*errvecs[jj]);
          smat(ii, jj) = smat(jj, ii) = trialvecs[ii]->dot(*trialvecs[jj]);
        }
      }
      ...
      for (int ii = 0; ii != iiter; ++ii) {
        ...
        val = std::fabs(val) < 1e-12 ? 1.0 / std::sqrt(std::fabs(val)) :
                                       1.0 / std::sqrt(val);
      }
      ...
      auto bnew = dbcsr::matrix<>::create_template(*b_ov).name("b_new").build();
