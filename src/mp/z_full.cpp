...
void LLMP_FULL_Z::compute()
{
  ...
  auto spgrid3_xob =
      dbcsr::pgrid<3>::create(m_cart.comm()).tensor_dims(xobsizes).build();
  ...
  auto b_xob_2_01 = dbcsr::tensor<3>::create_template(*b_xob_1_02)
                        .name("b_xob_2_01")
                        .map1({2})
                        .map2({0, 1})
                        .build();

  auto b2_xob_2_01 = dbcsr::tensor<3>::create_template(*b_xob_1_02)
                         .name("b2_xob_2_01")
                         .map1({2})
                         .map2({0, 1})
                         .build();

  auto b2_xob_1_02 = dbcsr::tensor<3>::create_template(*b_xob_1_02)
                         .name("b2_xob_1_02")
                         .map1({1})
                         .map2({0, 2})
                         .build();
  ...
  auto b2_xbb_0_12 = dbcsr::tensor<3>::create_template(*b2_xbb_1_02)
                         .name("b2_xbb_0_12")
                         .map1({0})
                         .map2({1, 2})
                         .build();
  ...
  for (int ix = 0; ix != m_eri3c2e_batched->nbatches(0); ++ix) {
    ...
    for (int inu = 0; inu != m_eri3c2e_batched->nbatches(2); ++inu) {
      ...
      vec<vec<int>> x_u_bounds = {
          m_eri3c2e_batched->bounds(0, ix), vec<int>{0, notot - 1}};
      ...
      vec<vec<int>> x_nu_bounds = {
          m_eri3c2e_batched->bounds(0, ix), m_eri3c2e_batched->bounds(2, inu)};
        ...
        for (int mublk = 0; mublk != (int)b.size(); ++mublk) {
          for (int nublk = bblkbounds[0]; nublk != bblkbounds[1] + 1; ++nublk) {
            ...
            for (int xblk = xblkbounds[0]; xblk != xblkbounds[1] + 1; ++xblk) {
              ...
              if (m_cart.rank() != b2_xbb_1_02->proc(idx))
                continue;
...
void LL_FULL_Z::init()
{
  ...
  m_zmat = dbcsr::matrix<double>::create()
               .set_cart(m_cart)
               .name("zmat")
               .row_blk_sizes(x)
               .col_blk_sizes(x)
...
void LL_FULL_Z::compute()
{
  ...
  auto spgrid3_xov =
      dbcsr::pgrid<3>::create(m_cart.comm()).tensor_dims(xovsizes).build();
  ...
  auto b_xov_2_01 = dbcsr::tensor<3>::create()
                        .set_pgrid(*spgrid3_xob)
                        .name("b_xov_1_02")
                        .map1({2})
                        .map2({0, 1})
  ...
  auto b_xov_0_12 = dbcsr::tensor<3>::create_template(*b_xov_2_01)
                        .name("b_xov_0_12")
                        .map1({0})
                        .map2({1, 2})
                        .build();
  ...
  int nbbatches = m_eri3c2e_batched->nbatches(1);

  std::array<int, 3> bdims_xov = {nxbatches, nbbatches, nbbatches};

  auto b_xov_batched = dbcsr::btensor<3>::create()
                           .name("b_xov_batched")
                           .set_pgrid(spgrid3_xov)
                           .blk_sizes(xov)
                           .blk_maps(blkmaps_xov)
  ...
  b_xov_batched->compress_init({0, 2}, {0}, {1, 2});
  ...
  for (int ix = 0; ix != m_eri3c2e_batched->nbatches(0); ++ix) {
    ...
    for (int iv = 0; iv != b_xov_batched->nbatches(2); ++iv) {
      LOG.os<1>("---- (IV) Batch ", iv, "\n");

      // second transform
      LOG.os<1>("---- Second transform.\n");

      vec<vec<int>> v_bounds = {b_xov_batched->bounds(2, iv)};
      ...
      time_tran2.start();
      dbcsr::contract(1.0, *m_lvir_01, *b_xob_2_01, 0.0, *b_xov_2_01)
          .bounds2(v_bounds)
          .bounds3(x_u_bounds)
          .filter(dbcsr::global::filter_eps)
          .perform("Na, XiN -> Xia");
      ...
      LOG.os<1>("---- Reordering B_xov.\n");
      time_reo2.start();
      dbcsr::copy(*b_xov_2_01, *b_xov_0_12).move_data(true).perform();
      time_reo2.finish();

      LOG.os<1>("---- Writing B_xov to memory.\n");
      time_write.start();
      b_xov_batched->compress({ix, iv}, b_xov_0_12);
      time_write.finish();
    }
  ...
  m_eri3c2e_batched->decompress_finalize();
  b_xov_batched->compress_finalize();

  LOG.os<1>("Occupation of b_xov: ", b_xov_batched->occupation() * 100, "%\n");
  ...
  time_setview.start();
  b_xov_batched->decompress_init({0, 2}, vec<int>{0}, vec<int>{1, 2});
  time_setview.finish();

  LOG.os<1>("Computing Z_XY.\n");

  for (int iv = 0; iv != b_xov_batched->nbatches(2); ++iv) {
    LOG.os<1>("Batch iv: ", iv, '\n');
    LOG.os<1>("Fetching integrals...\n");

    for (int ix = 0; ix != b_xov_batched->nbatches(0); ++ix) {
      LOG.os<1>("-- Batch ix: ", ix, '\n');

      LOG.os<1>("-- Fetching intermediate...\n");

      time_read.start();
      b_xov_batched->decompress({ix, iv});
      time_read.finish();

      auto b_xov_0_12 = b_xov_batched->get_work_tensor();

      vec<vec<int>> x_bounds = {b_xov_batched->bounds(0, ix)};

      vec<vec<int>> ov_bounds = {
          b_xov_batched->full_bounds(1), b_xov_batched->bounds(2, iv)};

      // form Z
      LOG.os<1>("-- Forming Z.\n");

      time_formz.start();
      dbcsr::contract(1.0, *b_xov_0_12, *b_xov_0_12, 1.0, *m_zmat_01)
          .bounds1(ov_bounds)
          .bounds2(x_bounds)
          .filter(dbcsr::global::filter_eps)
          .perform("Mia, Nia -> MN");
  ...
  b_xov_batched->decompress_finalize();
