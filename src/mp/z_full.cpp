...
void LL_FULL_Z::compute() {
	...
	auto b_xov_0_12 = 
		dbcsr::tensor<3>::create_template(*b_xov_2_01)
		.name("b_xov_0_12")
		.map1({0})
		.map2({1,2})
		.build();
	...
	for (int ix = 0; ix != m_eri3c2e_batched->nbatches(0); ++ix) {
		...
		for (int iv = 0; iv != b_xov_batched->nbatches(2); ++iv) {
			...
			vec<vec<int>> v_bounds = { 
				b_xov_batched->bounds(2,iv)
			};
	...
	for (int iv = 0; iv != b_xov_batched->nbatches(2); ++iv) {
		...
		for (int ix = 0; ix != b_xov_batched->nbatches(0); ++ix) {
			...
			vec<vec<int>> x_bounds = {
				b_xov_batched->bounds(0,ix)
			};
			
			vec<vec<int>> ov_bounds = {
				b_xov_batched->full_bounds(1),
				b_xov_batched->bounds(2,iv)
			};
