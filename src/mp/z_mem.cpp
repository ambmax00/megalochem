...
void LLMP_MEM_Z::compute()
{
  ...
  auto b2_xbb_1_02 = dbcsr::tensor<3>::create_template(*eri_1_02)
                         .name("b2_xbb_1_02")
                         .map1({1})
                         .map2({0, 2})
                         .build();

  auto b2_xbb_0_12 = dbcsr::tensor<3>::create_template(*eri_1_02)
                         .name("b2_xbb_0_12")
                         .map1({0})
                         .map2({1, 2})
                         .build();
  ...
  for (int ix = 0; ix != nxbatches; ++ix) {
    ...
    for (int inu = 0; inu != nnbatches; ++inu) {
      ...
      time_reo1.start();
      dbcsr::copy(*b_xob_1_02, *b_xob_2_01).move_data(true).sum(true).perform();
      time_reo1.finish();
    }
    ...
    for (int iy = 0; iy != m_eri3c2e_batched->nbatches(0); ++iy) {
      ...
      vec<vec<int>> y_bounds = {m_eri3c2e_batched->bounds(0, iy)};
...
void LLMP_ASYM_Z::compute() {
        ...
        for (int ix = 0; ix != nxbatches; ++ix) {
                ...
                for (int inu = 0; inu != nnbatches; ++inu) {
                        ...
                        time_tran1.start();
                        dbcsr::contract(1.0, *m_locc_01, *t_1_02, 0.0,
*b_xob_1_02) .bounds3(x_nu_bounds) .perform("mi, Xmn -> Xin");
                        time_tran1.finish();
                ...
                for (int inu = 0; inu != nnbatches; ++inu) {
                        ...
                        time_tran2.start();
                        dbcsr::contract(1.0, *m_pvir_01, *b_xob_2_01, 0.0,
*b2_xob_2_01) .bounds2(nu_bounds).bounds3(x_u_bounds)
                                .filter(dbcsr::global::filter_eps)
                                .perform("Nn, Xin -> XiN");
                        time_tran2.finish();
                        ...
                        LOG.os<1>("---- Reordering B_xoB.\n");
                        time_reo2.start();
                        dbcsr::copy(*b2_xob_2_01,
*b2_xob_1_02).move_data(true).perform(); time_reo2.finish();
                        ...
                        if (true) {
                                ...
                                auto xblkbounds =
m_t3c2e_left_batched->blk_bounds(0,ix); auto bblkbounds =
m_t3c2e_left_batched->blk_bounds(1,inu);
...
continue;
                                                ...
                                                for (int xblk = xblkbounds[0];
xblk != xblkbounds[1]+1; ++xblk) {

                                                        std::array<int,3> idx =
{xblk,mublk,nublk}; if (m_cart.rank() != b2_xbb_1_02->proc(idx)) continue;
                        ...
                        time_tran3.start();
                        dbcsr::contract(1.0, *m_locc_01, *b2_xob_1_02, 0.0,
*b2_xbb_1_02) .bounds3(x_nu_bounds) .retain_sparsity(force_sparsity)
                                .perform("Mi, XiN -> XMN");
                        time_tran3.finish();
                ...
                for (int iy = 0; iy != m_t3c2e_right_batched->nbatches(0); ++iy)
{
                        ...
                        time_formz.start();
                        dbcsr::contract(1.0, *b2_xbb_0_12, *eri_0_12, 1.0,
*m_zmat_01) .bounds2(x_bounds) .bounds3(y_bounds)
                                .filter(dbcsr::global::filter_eps)
                                .perform("Mmn, Nmn -> MN");
                        time_formz.finish();
