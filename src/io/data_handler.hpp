...
enum class create_mode { truncate, append };

enum class access_mode { rdwr, rdonly };
...
class data_handler {
 ...
 public:
  data_handler(std::string filename, create_mode cmode, MPI_Comm comm) :
      _filename(filename), _abs_filename(fs::absolute(filename)), _comm(comm),
      _is_open(false)
  {
    ...
    if (cmode == create_mode::truncate) {
      if (_rank == 0 && fs::exists(_filename))
        fs::remove(_filename);
      auto file_id = H5Fcreate(
          _abs_filename.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, _plist_id);
      H5Fclose(file_id);
    }
    else {
      if (!fs::exists(_filename))
        throw std::runtime_error(
            "Datahandler: file " + _abs_filename + " does not exist!");
    }
  }
  ...
  void open(access_mode amode)
  {
    ...
    unsigned h5mode =
        (amode == access_mode::rdonly) ? H5F_ACC_RDONLY : H5F_ACC_RDWR;
  ...
  {
    auto group = H5Gcreate(
        _file_id, gname.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    H5Gclose(group);
  }
  ...
  void write(
      std::string vname, T* data, std::vector<hsize_t> dims, int wrank = 0)
  {
    ...
    auto dset = H5Dcreate(
        _file_id, vname.c_str(), filetype, space, H5P_DEFAULT, H5P_DEFAULT,
        H5P_DEFAULT);

    if (wrank == _rank) {
      H5Dwrite(dset, memtype, H5S_ALL, H5S_ALL, data_plist, data);
    }
      ...
      std::string vname, T* data, std::vector<hsize_t> dims, int wrank = 0)
  {
    ...
    for (hsize_t ii = 0; ii != ntot; ++ii) {
      ...
      if (data[ii].size() > _max_strlength - 1) {
        throw std::runtime_error("Datahandler: string exceeds size limit.");
      }
    }

    auto dset = H5Dcreate(
        _file_id, vname.c_str(), strtype, space, H5P_DEFAULT, H5P_DEFAULT,
        H5P_DEFAULT);

    if (wrank == _rank) {
      H5Dwrite(dset, strtype, H5S_ALL, H5S_ALL, data_plist, c_strings.data());
    }
  ...
  data_array<T> read(std::string name)
  {
    ...
    H5Dread(dataset, memtype, H5S_ALL, H5S_ALL, data_plist, buf.data());
  ...
  data_array<T> read(std::string name)
  {
    ...
    H5Dread(dataset, strtype, H5S_ALL, H5S_ALL, data_plist, readbuf.data());

    H5Dclose(dataset);
    H5Sclose(space);

    for (hsize_t ii = 0; ii != ntot; ++ii) { outbuf[ii] = readbuf[ii].data; }
