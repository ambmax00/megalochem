...
inline void write_molden(
	std::string filename, world w, desc::molecule& mol, 
	dbcsr::matrix<double>& c_bo, dbcsr::matrix<double>& c_bv,
			...
			for (int iorb = 0; iorb != norb; ++iorb) {
				...
				for (int icluster = 0; icluster != cbas->size(); ++icluster) {
					...
					for (int ishell = 0; ishell != c.size(); ++ishell) {
							...
							switch (s.l) {
								case 0: map = smap; break;
								case 1: map = pmap; break;
								case 2: map = dmap; break;
								case 3: map = fmap; break;
								case 4: map = gmap; break;
							}
							
							for (int ii = 0; ii != size; ++ii) {
								file << noff + ii + 1 << " " 
									<< cmat(noff + map[ii],iorb) << '\n';
							}
