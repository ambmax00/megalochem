...
int main(int argc, char** argv)
{
  ...
  LOG.os<>(
      "Running ", mega_world.size(), " MPI processes with ",
  ...
  if (std::filesystem::exists(hdf5file)) {
    ...
    dh_in = std::make_shared<filio::data_handler>(
        hdf5backup, filio::create_mode::append, comm);
  }

  dh_out = std::make_shared<filio::data_handler>(
      hdf5file, filio::create_mode::truncate, comm);
