...
class screener {
 ...
 public:
  screener(world w, desc::shared_molecule mol, std::string method) :
      m_world(w), m_cart(w.dbcsr_grid()), m_mol(mol), m_method(method),
      m_fac(mol, w)
  {
  }
...
class atomic_screener : public screener {
 ...
 public:
  atomic_screener(world w, desc::shared_molecule mol, std::vector<int> alist) :
      screener(w, mol, "atomic"), m_atom_list(alist), m_schwarz(w, mol)
  {
  }
  ...
  std::vector<bool> blklist_b()
  {
    return m_blklist_b;
  }
  std::vector<bool> blklist_x()
  {
    return m_blklist_x;
  }
