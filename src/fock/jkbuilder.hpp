...
enum class jmethod { exact, dfao };

enum class kmethod { exact, dfao, dfmo, dfmem, dfrobust, dflmo };
...
class JK_common {
 ...
 public:
  JK_common(
      megalochem::world w,
      desc::shared_molecule smol,
      int print,
      std::string name);

  void set_density_alpha(dbcsr::shared_matrix<double>& ipA)
  {
    m_p_A = ipA;
  }

  void set_density_beta(dbcsr::shared_matrix<double>& ipB)
  {
    m_p_B = ipB;
  }

  void set_coeff_alpha(dbcsr::shared_matrix<double>& icA)
  {
    m_c_A = icA;
  }

  void set_coeff_beta(dbcsr::shared_matrix<double>& icB)
  {
    m_c_B = icB;
  }
...
class J : public JK_common {
 ...
 public:
  ...
  dbcsr::shared_matrix<double> get_J()
  {
    return m_J;
  }
};
...
class K : public JK_common {
 ...
 public:
  ...
  dbcsr::shared_matrix<double> get_K_A()
  {
    return m_K_A;
  }
  dbcsr::shared_matrix<double> get_K_B()
  {
    return m_K_B;
  }
};
...
#define BASE_LIST \
  (((megalochem::world), set_world), ((desc::shared_molecule), molecule), \
...
class EXACT_J : public J {
 ...
 public:
#define EXACT_J_LIST (((dbcsr::sbtensor<4, double>), eri4c2e_batched))
  ...
  EXACT_J(create_pack&& p) :
      BASE_INIT(J, EXACT_J), m_eri4c2e_batched(p.p_eri4c2e_batched)
  {
  }
...
class EXACT_K : public K {
 ...
 public:
#define EXACT_K_LIST (((dbcsr::sbtensor<4, double>), eri4c2e_batched))

  MAKE_PARAM_STRUCT(create, CONCAT(BASE_LIST, EXACT_J_LIST), ())
  MAKE_BUILDER_CLASS(EXACT_K, create, CONCAT(BASE_LIST, EXACT_J_LIST), ())

  EXACT_K(create_pack&& p) :
      BASE_INIT(K, EXACT_K), m_eri4c2e_batched(p.p_eri4c2e_batched)
  {
  }
...
#define DF_J_LIST \
  ...
  DF_J(create_pack&& p) :
      BASE_INIT(J, DF_J), m_eri3c2e_batched(p.p_eri3c2e_batched),
...
#define DFMO_K_LIST \
  ...
  DFMO_K(create_pack&& p) :
      BASE_INIT(K, DFMO_K), m_eri3c2e_batched(p.p_eri3c2e_batched),
...
#define DFAO_K_LIST \
  ...
  DFAO_K(create_pack&& p) :
      BASE_INIT(K, DFAO_K), m_eri3c2e_batched(p.p_eri3c2e_batched),
...
#define DFROBUST_K_LIST \
  ...
  DFROBUST_K(create_pack&& p) :
      BASE_INIT(K, DFROBUST_K), m_eri3c2e_batched(p.p_eri3c2e_batched),
      m_fitting_batched(p.p_fitting_batched), m_v_xx(p.p_metric)
  {
  }
...
#define DFMEM_K_LIST \
  ...
  DFMEM_K(create_pack&& p) :
      BASE_INIT(K, DFMEM_K), m_eri3c2e_batched(p.p_eri3c2e_batched),
...
#define DFLMO_K_LIST \
  ...
  DFLMO_K(create_pack&& p) :
      BASE_INIT(K, DFLMO_K), m_eri3c2e_batched(p.p_eri3c2e_batched),
      m_v_xx(p.p_metric_inv),
      m_occ_nbatches((p.p_occ_nbatches) ? *p.p_occ_nbatches : 5)
  {
  }
...
#define CREATE_J_LIST \
  (((megalochem::world), set_world), ((desc::shared_molecule), molecule), \
   ((jmethod), method), ((ints::aoloader), aoloader), \
   ((util::optional<int>), print), ((ints::metric), metric))
...
inline create_j_base create_j()
{
  return create_j_base();
}

#define CREATE_K_LIST \
  (((megalochem::world), set_world), ((desc::shared_molecule), molecule), \
   ((kmethod), method), ((ints::aoloader), aoloader), \
   ((util::optional<int>), print), ((ints::metric), metric), \
   ((util::optional<int>), occ_nbatches))
 ...
 public:
  ...
  std::shared_ptr<K> build()
  {
    ...
    if (*c_method == kmethod::exact) {
      ...
      auto invsqrt =
          aoreg.get<dbcsr::shared_matrix<double>>(ints::key::coul_xx_invsqrt);
...
inline create_k_base create_k()
{
  return create_k_base();
}
