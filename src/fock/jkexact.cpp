...
void EXACT_J::init()
{
  ...
  m_spgrid_bbd =
      dbcsr::pgrid<3>::create(m_cart.comm()).tensor_dims(tsizes).build();
  ...
  m_ptot_bbd =
      dbcsr::tensor<3>::create_template(*m_J_bbd).name("p dummy").build();
}
...
void EXACT_K::init()
{
  ...
  m_p_bbd = dbcsr::tensor<3>::create_template(*m_K_bbd).name("p dummy").build();
}
  ...
  for (int imu = 0; imu != m_eri4c2e_batched->nbatches(2); ++imu) {
    for (int inu = 0; inu != m_eri4c2e_batched->nbatches(3); ++inu) {
      ...
      vec<vec<int>> ls_bounds = {
          m_eri4c2e_batched->bounds(2, imu), m_eri4c2e_batched->bounds(3, inu)};

      dbcsr::contract(1.0, *m_ptot_bbd, *eri_01_23, 1.0, *m_J_bbd)
          .bounds1(ls_bounds)
          .perform("LS_, MNLS -> MN_");
    }
  }
