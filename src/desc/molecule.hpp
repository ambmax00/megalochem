  ...
  class block_sizes {
   ...
   public:
    ...
    block_sizes(molecule& mol, int nsplit)
    {
      ...
      m_tot_alpha_sizes.insert(
          m_tot_alpha_sizes.end(), m_vir_alpha_sizes.begin(),
          m_vir_alpha_sizes.end());

      m_tot_beta_sizes.insert(
          m_tot_beta_sizes.end(), m_vir_beta_sizes.begin(),
          m_vir_beta_sizes.end());
    ...
    std::vector<int> oa()
    {
      return m_occ_alpha_sizes;
    }
    std::vector<int> ob()
    {
      return m_occ_beta_sizes;
    }
    std::vector<int> va()
    {
      return m_vir_alpha_sizes;
    }
    std::vector<int> vb()
    {
      return m_vir_beta_sizes;
    }
    std::vector<int> ma()
    {
      return m_tot_alpha_sizes;
    }
    std::vector<int> mb()
    {
      return m_tot_beta_sizes;
    }
    std::vector<int> b()
    {
      return m_bas_sizes;
...
#define MOLECULE_CREATE_LIST \
  (((MPI_Comm), comm), ((std::string), name), ((std::vector<Atom>), atoms), \
   ((int), charge), ((int), mult), ((util::optional<int>), mo_split), \
  ...
  std::shared_ptr<desc::molecule> fragment(
      int noa, int nob, int nvo, int nvb, std::vector<int> atom_list);
};

using shared_molecule = std::shared_ptr<molecule>;

inline void write_molecule(
    std::string name, desc::molecule& mol, filio::data_handler& dh)
{
