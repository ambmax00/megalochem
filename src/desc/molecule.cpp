  ...
  if (m_frac) {
    if (m_atoms.size() != 1)
      throw std::runtime_error(
          "Fractional Occupation only for single atoms for now.");
...
void molecule::print_info(int level)
{
  ...
  LOG.os<1>("occ. alpha: ");
  printvec(LOG, 1, m_blocks.oa());
  LOG.os<1>("occ. beta: ");
  printvec(LOG, 1, m_blocks.ob());
  LOG.os<1>("vir. alpha: ");
  printvec(LOG, 1, m_blocks.va());
  LOG.os<1>("vir. beta: ");
  printvec(LOG, 1, m_blocks.vb());
  LOG.os<1>("basis: ");
  printvec(LOG, 1, m_blocks.b());

  if (m_cluster_dfbasis) {
    LOG.os<1>("dfbasis: ");
    printvec(LOG, 1, m_blocks.x());
  }

  if (m_frac_occ_alpha) {
    LOG.os<1>("Frac. occ. alpha: ");
    printvec(LOG, 1, *m_frac_occ_alpha);
  }

  if (m_frac_occ_beta) {
    LOG.os<1>("Frac. occ. beta: ");
    printvec(LOG, 1, *m_frac_occ_beta);
  }
    ...
    int noa, int nob, int nva, int nvb, std::vector<int> atom_list)
{
  ...
  for (auto iatom : atom_list) { frag->m_atoms.push_back(m_atoms[iatom]); }
  ...
  auto get_block = [&frag](desc::cluster_basis& cbas) {
    ...
    for (size_t ishell = 0; ishell != is_included.size(); ++ishell) {
      if (is_included[ishell])
        blklist.push_back(ishell);
    }
