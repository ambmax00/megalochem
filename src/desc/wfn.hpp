...
class hf_wavefunction {
 ...
 public:
  hf_wavefunction(
      ...
      double wfn_energy) :
      m_c_bo_A(c_bo_A),
      m_c_bv_A(c_bv_A), m_c_bo_B(c_bo_B), m_c_bv_B(c_bv_B),
...
class mp_wavefunction {
 ...
 public:
  mp_wavefunction(double mp_os_energy, double mp_ss_energy, double mp_energy) :
      m_mp_os_energy(mp_os_energy), m_mp_ss_energy(mp_ss_energy),
  ...
  double mp_os_energy()
  {
    return m_mp_os_energy;
  }
  double mp_ss_energy()
  {
    return m_mp_ss_energy;
  }
  double mp_energy()
  {
    return m_mp_energy;
...
class adc_wavefunction {
 ...
 public:
  adc_wavefunction(
      ...
      m_dav_eigvecs(dav_eigvecs), m_blocked(blocked)
  {
  }
...
inline void write_mat(
    filio::data_handler& dh, std::string name, dbcsr::shared_matrix<double> mat)
{
  if (mat) {
    ...
    bool throw_if_not_present)
{
  ...
  Eigen::Map<MatrixX<double, Eigen::RowMajor>> emap(
      darray.data.data(), nrows, ncols);

  auto mat =
      dbcsr::eigen_to_matrix(emap, g, name, r, c, dbcsr::type::no_symmetry);
...
inline void write_hfwfn(
    std::string name, hf_wavefunction& hfwfn, filio::data_handler& dh)
{
...
inline desc::shared_hf_wavefunction read_hfwfn(
    std::string name,
    ...
    filio::data_handler& dh)
{
  ...
  auto hfwfn = std::make_shared<desc::hf_wavefunction>(
      c_bo_A, c_bo_B, c_bv_A, c_bv_B, eps_occ_A, eps_occ_B, eps_vir_A,
      eps_vir_B, scf_energy, nuc_energy, wfn_energy);
...
inline void write_adcwfn(
    std::string name, adc_wavefunction& adcwfn, filio::data_handler& dh)
{
  dh.open(filio::access_mode::rdwr);
...
inline desc::shared_adc_wavefunction read_adcwfn(
    std::string name,
    desc::shared_molecule mol,
    world w,
    filio::data_handler& dh)
{
  ...
  for (int iroot = 0; iroot != nroots; ++iroot) {
    ...
    eigvecs[iroot] =
        read_mat(dh, statename + "/vector", w.dbcsr_grid(), oa, va, true);
  }

  auto adcwfn =
      std::make_shared<desc::adc_wavefunction>(true, eigvals, eigvecs);
