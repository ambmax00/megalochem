...
void MVP_AORISOSADC2::init()
{
  ...
  m_s_sqrt_bb = chol.L(m_b);
  m_s_invsqrt_bb = chol.L_inv(m_b);
  ...
  m_laphelper_ss = std::make_shared<math::laplace_helper>(
      m_world, m_nlap, m_c_bo, m_c_bv, m_s_sqrt_bb, m_s_invsqrt_bb, m_eps_occ,
      m_eps_vir, LOG.global_plev());
  ...
  bool do_o = true;
  bool do_v =
      (m_kmethod == fock::kmethod::dflmo || m_zmethod == mp::zmethod::ll_full) ?
      true :
      false;

  m_laphelper_ss->compute(do_o, do_v, 0.0, m_mol->mo_split());
  ...
  switch (m_zmethod) {
    ...
    case mp::zmethod::ll_full: {
      m_zbuilder = mp::LL_FULL_Z::create()
                       .set_world(m_world)
                       .set_molecule(m_mol)
                       .print(nprint)
                       .eri3c2e_batched(m_eri3c2e_batched)
...
std::pair<smat, smat> MVP_AORISOSADC2::compute_jk(smat& u_ia, smat& u_ao)
{
...
void MVP_AORISOSADC2::compute_intermeds()
{
  ...
  auto i_oo_tmp =
      dbcsr::matrix<>::create_template(*m_i_oo).name("i_oo_temp").build();

  auto i_vv_tmp =
      dbcsr::matrix<>::create_template(*m_i_vv).name("i_vv_temp").build();

  dbcsr::shared_matrix<double> po, pv, lo, lv;
  ...
  for (int ilap = 0; ilap != m_nlap; ++ilap) {
    ...
    po = m_laphelper_ss->pseudo_densities_occ()[ilap];
    pv = m_laphelper_ss->pseudo_densities_vir()[ilap];

    lo = m_laphelper_ss->pseudo_cholesky_occ()[ilap];
    if (m_zmethod == mp::zmethod::ll_full ||
        m_kmethod == fock::kmethod::dflmo) {
      lv = m_laphelper_ss->pseudo_cholesky_vir()[ilap];
      lv->filter(dbcsr::global::filter_eps);
    }

    po->filter(dbcsr::global::filter_eps);
    pv->filter(dbcsr::global::filter_eps);

    m_zbuilder->set_occ_coeff(lo);
    m_zbuilder->set_vir_coeff(lv);
  ...
  dbcsr::multiply('N', 'N', 1.0, *i_ob, *m_c_bo, 0.0, *m_i_oo).perform();
  dbcsr::multiply('N', 'N', 1.0, *i_vb, *m_c_bv, 0.0, *m_i_vv).perform();
...
smat MVP_AORISOSADC2::compute_sigma_2a(smat& u_ia)
{
  ...
  // sig_2a = i_vv_ab * u_ib
  auto sig_2a = dbcsr::matrix<>::create_template(*u_ia).name("sig_2a").build();

  dbcsr::multiply('N', 'T', 1.0, *u_ia, *m_i_vv, 0.0, *sig_2a).perform();
...
smat MVP_AORISOSADC2::compute_sigma_2b(smat& u_ia)
{
  ...
  // sig_2b = i_oo_ij * u_ja
  auto sig_2b = dbcsr::matrix<>::create_template(*u_ia).name("sig_2b").build();

  dbcsr::multiply('N', 'N', 1.0, *m_i_oo, *u_ia, 0.0, *sig_2b).perform();
...
smat MVP_AORISOSADC2::compute_sigma_2c(smat& jmat, smat& kmat)
{
   ...
   * 				c_na * exp(-eps_a t) * (mn|X') * (X'|X)^inv
   * d_X(t) d_X(t) = (nk|X) * I_n'k' * Po_nn'(t) * Pv_kk'(t) I_mk = jmat +
   * transpose(K)
   */
  ...
  auto I_ao = dbcsr::matrix<>::create_template(*jmat).name("I_ao").build();
  auto jmat_t = dbcsr::matrix<>::transpose(*jmat).build();
  auto kmat_t = dbcsr::matrix<>::transpose(*kmat).build();
  ...
  for (int ilap = 0; ilap != m_nlap; ++ilap) {
    ...
    auto jmat_trans =
        u_transform(jmat_pseudo, 'T', c_bo_scaled, 'N', c_bv_scaled);
...
smat MVP_AORISOSADC2::compute_sigma_2d(smat& u_ia)
{
  ...
  for (int ilap = 0; ilap != m_nlap; ++ilap) {
    ...
    auto u_pseudo = u_transform(u_ia, 'N', c_bo_scaled, 'T', c_bv_scaled);
  ...
  if (m_kmethod == fock::kmethod::dflmo) {
    math::SVD svd(m_world, I_ao, 'V', 'V', LOG.global_plev());
    svd.compute();

    auto r = dbcsr::split_range(svd.rank(), m_mol->mo_split());
    auto b = I_ao->col_blk_sizes();

    auto s = svd.s();
    auto u_br = svd.U(b, r);
    auto vt_rb = svd.Vt(r, b);

    vt_rb->scale(s, "left");

    m_kbuilder->set_coeff_left_alpha(u_br);
    m_kbuilder->set_coeff_right_alpha(vt_rb);
  }
...
MVP_AORISOSADC2::compute_laplace_batchtensors_OB(
    smat& u_ao, smat& L_bo, smat& pv_bb)
{
  ...
  arrvec<int, 2> bo_chol = {m_b, o_chol};
  arrvec<int, 2> ob_chol = {o_chol, m_b};
  arrvec<int, 2> bb = {m_b, m_b};
  arrvec<int, 3> xbb = {m_x, m_b, m_b};
  arrvec<int, 3> xob_chol = {m_x, o_chol, m_b};
  ...
  auto spgrid_xob_chol =
      dbcsr::pgrid<3>::create(m_cart.comm()).tensor_dims(dims_xob_chol).build();
  ...
  auto SL_bo = dbcsr::matrix<>::create_template(*L_bo).name("SL_bo").build();
  ...
  dbcsr::multiply('N', 'N', 1.0, *m_s_bb, *L_bo, 0.0, *SL_bo).perform();

  dbcsr::multiply('N', 'N', 1.0, *m_s_bb, *pv_bb, 0.0, *SPv_bb).perform();

  dbcsr::multiply('T', 'N', 1.0, *SL_bo, *u_ao, 0.0, *up_ob).perform();

  dbcsr::multiply('N', 'N', 1.0, *u_ao, *SPv_bb, 0.0, *uh_bb).perform();
  ...
  for (int ix = 0; ix != nxbatches; ++ix) {
    ...
    vec<vec<int>> xn_bounds = {
        m_eri3c2e_batched->bounds(0, ix), m_eri3c2e_batched->full_bounds(2)};
    ...
    dbcsr::copy(*HT_xob_02_1, *HT_xob_01_2).move_data(true).perform();

    vec<vec<int>> xo_bounds = {
        m_eri3c2e_batched->bounds(0, ix), eri_xob_batched->full_bounds(1)};
    ...
    dbcsr::copy(*FT_xob_01_2, *FT_xob_02_1).move_data(true).perform();
    ...
    dbcsr::copy(*FT_xob_01_2, *J_xob_02_1).move_data(true).perform();
    ...
    dbcsr::copy(*FT_xob_01_2, *J_xob_02_1).sum(true).move_data(true).perform();
  ...
  LOG.os<1>(
      "Occupancies: ", J_xob_batched->occupation(), ", ",
...
MVP_AORISOSADC2::compute_F_OB(
    dbcsr::sbtensor<3, double> eri_xob_batched,
    ...
    dbcsr::shared_matrix<double> L_bo)
{
  ...
  auto FT_xbb_02_1 = m_eri3c2e_batched->get_template(
      "FT_xbb_02_1", vec<int>{0, 2}, vec<int>{1});

  auto FT_xbb_0_12 = m_eri3c2e_batched->get_template(
      "FT_xbb_0_12", vec<int>{0}, vec<int>{1, 2});
  ...
  auto FA_xx_01 =
      dbcsr::tensor<2>::create_template(*V_xx_01).name("FA_xx_01").build();

  auto FB_xx_01 =
      dbcsr::tensor<2>::create_template(*V_xx_01).name("FB_xx_01").build();
  ...
  for (int ib = 0; ib != nbbatches; ++ib) {
    ...
    vec<vec<int>> ob_bounds = {
        eri_xob_batched->full_bounds(1), eri_xob_batched->bounds(2, ib)};
    ...
    for (int ix = 0; ix != nxbatches; ++ix) {
      ...
      vec<vec<int>> xn_bounds = {
          m_eri3c2e_batched->bounds(0, ix), m_eri3c2e_batched->bounds(2, ib)};

      vec<vec<int>> mn_bounds = {
          m_eri3c2e_batched->full_bounds(1), m_eri3c2e_batched->bounds(2, ib)};
      ...
      dbcsr::copy(*FT_xbb_02_1, *FT_xbb_0_12).move_data(true).perform();
  ...
  auto temp = dbcsr::tensor<2>::create_template(*V_xx_01).name("temp").build();

  auto transform = [&](auto& F) {
    dbcsr::contract(1.0, *V_xx_01, *F, 0.0, *temp).perform("XY, YZ -> XZ");

    dbcsr::contract(1.0, *temp, *V_xx_01, 0.0, *F).perform("XY, YZ -> XZ");
  };
...
dbcsr::sbtensor<3, double> MVP_AORISOSADC2::compute_I_OB(
    dbcsr::sbtensor<3, double>& eri_xob_batched,
    dbcsr::sbtensor<3, double>& R_xob_batched,
    dbcsr::shared_tensor<2, double>& F_A,
    dbcsr::shared_tensor<2, double>& F_B)
{
  ...
  auto I_xob_batched = dbcsr::btensor<3>::create_template(*eri_xob_batched)
                           .name("I_xob_batched")
                           .btensor_type(m_btype)
                           .print(LOG.global_plev())
                           .build();
  ...
  auto I_xob_01_2 =
      I_xob_batched->get_template("I_xbb_01_2", vec<int>{0, 1}, vec<int>{2});

  auto I_xob_0_12 =
      I_xob_batched->get_template("I_xbb_0_12", vec<int>{0}, vec<int>{1, 2});
  ...
  for (int ib = 0; ib != nbbatches; ++ib) {
    ...
    for (int ix = 0; ix != nxbatches; ++ix) {
      ...
      vec<vec<int>> xbounds = {xbds};

      vec<vec<int>> kabounds = {fullobds, bbds};
    ...
    dbcsr::copy(*I_xob_0_12, *I_xob_01_2).move_data(true).perform();
    ...
    [[maybe_unused]] double omega)
{
  ...
  auto I_xob_02_1 =
      I_xob_batched->get_template("I_xob_02_1", vec<int>{0, 2}, vec<int>{1});
  ...
  for (int ix = 0; ix != nxbatches; ++ix) {
    ...
    for (int ib = 0; ib != nbbatches; ++ib) {
      ...
      vec<vec<int>> xobounds = {
          m_eri3c2e_batched->bounds(0, ix), I_xob_batched->full_bounds(1)};

      vec<vec<int>> mbounds = {m_eri3c2e_batched->bounds(2, ib)};
      ...
      // form sig_B
      vec<vec<int>> xob_bds = {
          I_xob_batched->bounds(0, ix), I_xob_batched->full_bounds(1),
          I_xob_batched->bounds(2, ib)};

      dbcsr::copy(*I_xob_01_2, *I_xob_02_1).bounds(xob_bds).perform();
  ...
  dbcsr::multiply('N', 'N', 1.0, *m_s_bb, *m_c_bv, 0.0, *SC_bv).perform();

  dbcsr::multiply('N', 'N', 1.0, *m_s_bb, *m_c_bo, 0.0, *SC_bo).perform();

  dbcsr::multiply('T', 'N', 1.0, *L_bo, *SC_bo, 0.0, *LSC_co).perform();
...
smat MVP_AORISOSADC2::compute_sigma_2e_OB(smat& u_ao, double omega)
{
   ...
   *
   * Ftilde_{X'Y'}(t) = (X'|X) (X|μν) * R_{Xμ'ν'} * Po(t)_{μμ'} * Pv(t)_{νν'}
   * (YY')
   *
  ...
  auto sigma_2e_B =
      dbcsr::matrix<>::create_template(*sigma_2e_A).name("sigma_2e_B").build();
  ...
  for (int ilap = 0; ilap != m_nlap; ++ilap) {
    ...
    auto I_xob_batched =
        compute_I_OB(eri_xob_batched, J_xob_batched, FA_xx, FB_xx);

    J_xob_batched->reset();
    eri_xob_batched->reset();

    auto [sig_ilap_E1, sig_ilap_E2] =
        compute_sigma_2e_ilap_OB(I_xob_batched, L_bo, omega);
...
std::tuple<dbcsr::sbtensor<3, double>, dbcsr::sbtensor<3, double>>
MVP_AORISOSADC2::compute_laplace_batchtensors_OV(
    smat& u_ao, smat& L_bo, smat& L_bv)
{
  LOG.os<1>("Compute J and Teri\n");
  ...
  arrvec<int, 2> bo_chol = {m_b, o_chol};
  arrvec<int, 2> ob_chol = {o_chol, m_b};
  arrvec<int, 2> bv_chol = {m_b, v_chol};
  arrvec<int, 2> bb = {m_b, m_b};
  arrvec<int, 3> xbb = {m_x, m_b, m_b};
  arrvec<int, 3> xob_chol = {m_x, o_chol, m_b};
  arrvec<int, 3> xov_chol = {m_x, o_chol, v_chol};
  ...
  auto spgrid_xov_chol =
      dbcsr::pgrid<3>::create(m_cart.comm()).tensor_dims(dims_xov_chol).build();
  ...
  auto SL_bv = dbcsr::matrix<>::create_template(*L_bv).name("SL_bv").build();
  ...
  dbcsr::multiply('N', 'N', 1.0, *m_s_bb, *L_bv, 0.0, *SL_bv).perform();

  dbcsr::multiply('T', 'N', 1.0, *SL_bo, *u_ao, 0.0, *up_ob).perform();

  dbcsr::multiply('N', 'N', 1.0, *u_ao, *SL_bv, 0.0, *uh_bv).perform();
  ...
  for (int ix = 0; ix != nxbatches; ++ix) {
    ...
    vec<vec<int>> xo_bounds = {
        m_eri3c2e_batched->bounds(0, ix), eri_xov_batched->full_bounds(1)};
    ...
    dbcsr::copy(*FT_xov_01_2, *FT_xov_0_12).move_data(true).perform();
    ...
    dbcsr::copy(*FT_xov_01_2, *J_xov_0_12).move_data(true).perform();
    ...
    dbcsr::copy(*FT_xov_01_2, *J_xov_0_12).sum(true).move_data(true).perform();
  ...
  LOG.os<1>(
      "Occupancies: ", J_xov_batched->occupation(), ", ",
...
std::tuple<dbcsr::shared_tensor<2, double>, dbcsr::shared_tensor<2, double>>
MVP_AORISOSADC2::compute_F_OV(
    dbcsr::sbtensor<3, double> eri_xov_batched,
    dbcsr::sbtensor<3, double> J_xov_batched)
{
  ...
  for (int iv = 0; iv != nvbatches; ++iv) {
    ...
    vec<vec<int>> ov_bounds = {
        eri_xov_batched->full_bounds(1), eri_xov_batched->bounds(2, iv)};
...
dbcsr::sbtensor<3, double> MVP_AORISOSADC2::compute_I_OV(
    dbcsr::sbtensor<3, double>& eri_xov_batched,
    dbcsr::sbtensor<3, double>& R_xov_batched,
    dbcsr::shared_tensor<2, double>& F_A,
    dbcsr::shared_tensor<2, double>& F_B)
{
  auto& time = TIME.sub("Computing I batched");
  ...
  auto I_xov_batched = dbcsr::btensor<3>::create_template(*eri_xov_batched)
                           .name("I_xov_batched")
                           .btensor_type(m_btype)
                           .print(LOG.global_plev())
                           .build();
  ...
  auto I_xov_0_12 =
      I_xov_batched->get_template("I_xbb_0_12", vec<int>{0}, vec<int>{1, 2});
  ...
  for (int iv = 0; iv != nvbatches; ++iv) {
    ...
    for (int ix = 0; ix != nxbatches; ++ix) {
      ...
      vec<vec<int>> kabounds = {fullobds, vbds};
    ...
    [[maybe_unused]] double omega)
{
  ...
  arrvec<int, 2> ob_chol = {o_chol, m_b};
  arrvec<int, 3> xob_chol = {m_x, o_chol, m_b};
  arrvec<int, 3> xvb_chol = {m_x, v_chol, m_b};
  ...
  auto spgrid_xvb_chol =
      dbcsr::pgrid<3>::create(m_cart.comm()).tensor_dims(dims_xvb_chol).build();
  ...
  for (int ix = 0; ix != nxbatches; ++ix) {
    ...
    for (int io = 0; io != nobatches; ++io) {
      ...
      vec<vec<int>> obounds = {I_xov_batched->bounds(1, io)};
      ...
      vec<vec<int>> xobounds = {
          m_eri3c2e_batched->bounds(0, ix), I_xov_batched->bounds(1, io)};
  ...
  for (int ix = 0; ix != m_eri3c2e_batched->nbatches(0); ++ix) {
    ...
    for (int iv = 0; iv != nvbatches; ++iv) {
      vec<vec<int>> xmbounds = {
          m_eri3c2e_batched->bounds(0, ix), m_eri3c2e_batched->full_bounds(1)};
      vec<vec<int>> vbounds = {I_xov_batched->bounds(2, iv)};
      ...
      vec<vec<int>> xvbounds = {
          m_eri3c2e_batched->bounds(0, ix), I_xov_batched->bounds(2, iv)};
  ...
  dbcsr::multiply('N', 'T', 1.0, *m_c_bo, *m_c_bo, 0.0, *Po_bb).perform();

  dbcsr::multiply('N', 'T', 1.0, *m_c_bv, *m_c_bv, 0.0, *Pv_bb).perform();

  auto SC_bo = dbcsr::matrix<>::create_template(*m_c_bo).name("SC_bo").build();

  auto SC_bv = dbcsr::matrix<>::create_template(*m_c_bv).name("SC_bo").build();
...
smat MVP_AORISOSADC2::compute_sigma_2e_OV(smat& u_ao, double omega)
{
  ...
  for (int ilap = 0; ilap != m_nlap; ++ilap) {
    ...
    auto I_xov_batched =
        compute_I_OV(eri_xov_batched, J_xov_batched, FA_xx, FB_xx);

    J_xov_batched->reset();
    eri_xov_batched->reset();

    auto [sig_ilap_E1, sig_ilap_E2] =
        compute_sigma_2e_ilap_OV(I_xov_batched, L_bo, L_bv, omega);
...
smat MVP_AORISOSADC2::compute(smat u_ia, double omega)
{
  ...
  auto jkpair = compute_jk(u_ia, u_ao);
