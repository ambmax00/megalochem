...
enum class mvpmethod { ao_adc_1, ao_adc_2 };
...
#define AORIADC1_LIST \
  (((megalochem::world), set_world), ((desc::shared_molecule), set_molecule), \
   ...
   ((std::vector<double>), eps_occ), ((std::vector<double>), eps_vir), \
   ((dbcsr::sbtensor<3, double>), eri3c2e_batched), \
   ((dbcsr::sbtensor<3, double>), fitting_batched), \
   ((fock::kmethod), kmethod), ((fock::jmethod), jmethod), \
...
class MVP_AORISOSADC2 : public MVP {
 private:
  ...
  std::vector<double> m_weights, m_xpoints, m_weights_dd, m_xpoints_dd;

  double m_prev_omega = -1;

  double m_old_omega = -1.0;

  std::shared_ptr<math::laplace_helper> m_laphelper_ss, m_laphelper_dd;
  ...
  dbcsr::sbtensor<3, double> compute_I_OB(
      dbcsr::sbtensor<3, double>& eri,
      dbcsr::sbtensor<3, double>& J,
  ...
  dbcsr::sbtensor<3, double> compute_I_OV(
      dbcsr::sbtensor<3, double>& eri,
      dbcsr::sbtensor<3, double>& J,
      dbcsr::shared_tensor<2, double>& F_A,
      dbcsr::shared_tensor<2, double>& F_B);
...
#define AORIADC2_LIST \
  (((megalochem::world), set_world), ((desc::shared_molecule), set_molecule), \
   ...
   ((dbcsr::sbtensor<3, double>), eri3c2e_batched), \
   ((dbcsr::sbtensor<3, double>), fitting_batched), \
   ((util::optional<int>), nbatches_occ), ((fock::kmethod), kmethod), \
   ((mp::zmethod), zmethod), ((fock::jmethod), jmethod), \
   ((dbcsr::btype), btype), ((int), nlap), ((double), c_os), \
  ...
  MVP_AORISOSADC2(create_pack&& p) :
      MVP(p.p_set_world,
          ...
          (p.p_print) ? *p.p_print : 0,
          "MVP_AORISOSADC2"),
      m_s_bb(p.p_s_bb), m_c_bo(p.p_c_bo), m_c_bv(p.p_c_bv),
      m_v_xx(p.p_metric_inv), m_eri3c2e_batched(p.p_eri3c2e_batched),
      m_fitting_batched(p.p_fitting_batched), m_eps_occ(p.p_eps_occ),
      m_eps_vir(p.p_eps_vir), m_jmethod(p.p_jmethod), m_kmethod(p.p_kmethod),
      m_zmethod(p.p_zmethod), m_btype(p.p_btype), m_nlap((p.p_nlap)),
      m_c_os((p.p_c_os)), m_c_os_coupling((p.p_c_os_coupling)),
      m_nbatches_occ((p.p_nbatches_occ) ? *p.p_nbatches_occ : 5),
      m_o(p.p_c_bo->col_blk_sizes()), m_v(p.p_c_bv->col_blk_sizes()),
      m_b(p.p_c_bo->row_blk_sizes()), m_x(p.p_metric_inv->row_blk_sizes())
  {
  }
...
private:
        ...
        std::pair<smat,smat> compute_sigma_2e_ilap(dbcsr::sbtensor<3,double>&
J_xbb_batched, smat& FA, smat& FB, smat& pseudo_o, smat& pseudo_v #if 0 , double
omega, int ilap #endif
        );
