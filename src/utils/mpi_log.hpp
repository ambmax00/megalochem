...
class mpi_log {
 ...
 public:
  mpi_log(MPI_Comm comm, int glb) : global_plev_(glb), m_comm(comm)
  {
...
#define modifier(str) \
  ...
  modifier(fixed) modifier(scientific) modifier(hexfloat) modifier(defaultfloat)
      modifier(right) modifier(left)
  ...
  mpi_log& os(T in, Args... args)
  {
    if (nprint <= global_plev_) {
      if (m_mainproc == -1) {
        if (omp_in_parallel())
          throw std::runtime_error("ERROR: You should not use mpi_log for all "
                                   "processes (-1) in parallel regions.");
